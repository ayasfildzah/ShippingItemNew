...
public class ListInvoiceDetails {
    private static final String TAG = ListInvoiceDetails.class.getName();

    public ArrayList<InvoiceDetails> dm(Context context) throws IOException {
        try {
            HttpsTrustManager.allowAllSSL();
            HttpsURLConnection conection = GetHttpsURLConnection.getHttpsURLConnection(context, "invoice_recap_details");
            conection.setRequestMethod("GET");
            conection.setReadTimeout(READ_TIMEOUT);
            conection.setConnectTimeout(CONNECTION_TIMEOUT);
            conection.connect();
            ArrayList<InvoiceDetails> data1 = new ArrayList<InvoiceDetails>();
            if (conection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                try {
                    ...
                    for (int x = 0; x < ja.length(); x++) {
                        ...
                        InvoiceDetails model1 = new InvoiceDetails();
                        model1.setId(jo1.getString("id"));
                        model1.setInvoice_recap_number(jo1.getString("invoice_number"));
                        model1.setCreator(jo1.getString("creator"));
                        model1.setInvoice_recap_id(jo1.getString("invoice_recap_id"));
                        model.setInvoice_recap_number(jo1.getString("invoice_recap_number"));
